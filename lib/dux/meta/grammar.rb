...
module Dux
  # contains Dux::Rules and can apply them by validating XML or qualifying user input
  # reporting Dux::Errors to History as needed
  class Grammar < Object
    # Dux::Grammar can be initialized from XML Element or
    ...
    def sheet_to_xml(spreadsheet)
      ...
      worksheet.each_with_index do |row, index|
        ...
        self << Dux::ChildrenRule.new(element_name, statement_str)
        row[5].value.split(/\n/).each do |rule| # looping through attribute rules
          next if rule.empty?
          attr_name, value_expr, attr_req = *rule.split
          self << Dux::AttributesRule.new(element_name, attr_name, attr_req)
          self << Dux::ValueRule.new(element_name, attr_name, value_expr)
        end
