...
module DuxObjGuts
  ...
  def init_generic child
    self << DuxObject.new(child, reserved: reserved_word_array)
  end
  ...
  def coerce obj
    case obj.class
      when String                                       then DuxObject.new(Nokogiri::XML(obj).root)
      when Nokogiri::XML::Element                       then DuxObject.new(obj)
      when obj.respond_to?(:document?) && obj.document? then DuxObject.new(obj.root)
      else obj
    end
  end
  ...
  def resolve_ref attr, context_template=nil
    ...
    if context_template.nil?
      DuxObject.new File.open ref if File.exists?(ref)
    else
