require File.expand_path(File.dirname(__FILE__) + '/../../dux/meta/grammar')
require File.expand_path(File.dirname(__FILE__) + '/../../dux/dux_object')

class History < DuxObject
  include Enumerable
  ...
  def update type, change_hash
    change_class = Module::const_get type.to_s.classify
...
class Insert < Change
  def description
    super || %(Element '#{inserted.id}' of type '#{inserted.type}' was added to element '#{subject.id}' of type '#{subject.type}'.)
  end
