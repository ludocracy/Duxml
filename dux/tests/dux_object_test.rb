...
class DuxObjectTest < MiniTest::Test
  ...
  def test_init_nil
   assert_equal 'dux_object', DuxObject.new(nil).type
  end

  def test_init_str
    test_xml = Nokogiri::XML(%(<poop id="birdhouse0"/>)).root
    @e = DuxObject.new(test_xml.to_s)
    assert_equal test_xml.to_s, e.xml.to_s
  end

  def test_init_simple
    test_xml = Nokogiri::XML(%(<poop id="birdhouse0"/>)).root
    @e = DuxObject.new(test_xml)
    assert_equal test_xml.to_s, e.xml.to_s
  end
  ...
  def test_init_hierarchy
    @e = DuxObject.new(%(<poop id="birdhouse0"><danglers id="danglers0">dangling</danglers><chunks id="chunk0">chunky</chunks></poop>))
